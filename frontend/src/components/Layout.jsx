...
const Layout = ({ children }) => {
  ...
  const fetchUnreadCount = async () => {
    try {
      ...
      const openComplaints = response?.data?.filter(c => c.status === 'Open').length || 0;
      setUnreadCount(openComplaints);
    } catch (error) {
        ...
        <div className="p-4 border-t border-slate-700">
          ...
          {(!sidebarCollapsed || mobileMenuOpen) && (
            <button
              ...
              className="w-full mt-2 px-4 py-2 text-sm text-slate-300 hover:text-white hover:bg-slate-700/50 rounded-lg transition-colors flex items-center justify-center space-x-2"
            >
